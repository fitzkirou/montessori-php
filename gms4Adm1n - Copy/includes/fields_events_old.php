...
?>
	...
	<tr>
		...
		<?php
		/*	-----------------------------------------------------------------
		START OF THE EVENT-TYPE CODE
		------------------------------------------------------------------ */
			// Get the list of event types from the database.
			error_reporting (E_ALL);
			$types_query =  "SELECT * ";
			$types_query .= "FROM event_type";
			$types_result = mysql_query ($types_query);
			
			if ($types_result == FALSE) // DEBUGGING!
			{
				echo("<p style=\"color:#C00\">ERROR: Bob is having trouble ");
				echo("with the database table for event types. He hopes to ");
				echo("have it fixed soon!</p>");
			}
		?>
			<select name="eventList" size="1">
		<?php
			...
			{
				// Loop to show the type options.
				while ($type_row = mysql_fetch_array ($types_result))
				{
					echo ("<option value=\"" . $type_row['PK_typeID'] . "\">");
					echo ($type_row['type'] . "</option>\n");
				}
			}
		/*	-----------------------------------------------------------------
		END OF THE EVENT-TYPE CODE
		------------------------------------------------------------------ */
		?>
			</select>
