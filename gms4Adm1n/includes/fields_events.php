<?php
	...
	// Create an array of event-types and corresponding codes.
	// The key => value pairs read as "code => type of event".
	// The program iterates through this array on lines 103-119 below.
	//
	//
	// IMPORTANT: 
	// You must repeat this array at the bottom of file 'includes/library.php'.
	$types_array = array ("D" => "Designated Holiday",
						  "E" => "School Event", 
						  "H" => "Statutory Holiday", 
...
?>
	...
	<tr>
		<td colspan="2" valign="top">
			...
			<tr>
				<td valign="top" align="right">
					&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
					<strong>Show on homepage<br>&amp; newsletter?</strong>
				</td>
		...
		<?php
			...
			if (isset ($thisEventID))
			{
				// Get the code value that matches this $PK_eventID.
				$thisType_query = " SELECT code, type 
									FROM event
									WHERE PK_eventID = '$thisEventID'
									ORDER BY code ASC";
				$thisType_result = mysql_query ($thisType_query);
				if ($thisType_result)
				{
					while ($typeRow = mysql_fetch_array ($thisType_result))
					{
						$thisTypeCode = $typeRow['code'];
					}
				}
				else
				{
				...
				// Loop thru all the event types.
				foreach ($types_array AS $code => $eventType)
				{
					// Append the 'selected' attribute to the type row
					// that matches this PK_eventID.
					if ($code == $thisTypeCode)
					{
						echo("\t<option value=\"" . $code . '"');
						echo (' selected>' . $eventType);
						echo ("</option>\n");
					}
					else
					{
	...
	<tr>
		<td class="formLabel">Location/Comment: </td>
		<td>
		<?php
			// Only populate the field w/ data if we're updating and have the GET variable '$thisEventID'.
			if(isset($thisEventID))
	...
	<tr>
		...
		<?php
			...
			*/
			echo ('&nbsp;&nbsp;');
			echo ('<input type="Button" value="Delete" onclick="window.location.href=\'');
			echo ('event_delete.php?PK_eventID=' . $thisEventID . '\'">');
		?>
